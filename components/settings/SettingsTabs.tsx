...
export function SettingsTabs({ activeTab, onTabChange }: SettingsTabsProps) {
  ...
  return (
    <div className={`flex flex-col gap-1 w-full sm:w-64 transition-all duration-300 ${
      isRTL ? 'border-r border-gray-100' : 'border-l border-gray-100'
    }`}>
      {categories.map((cat) => (
        <button
          key={cat.id}
          ...
          className={`flex items-center gap-3 px-4 py-3 text-sm font-medium transition-all duration-200 ${
            activeTab === cat.id
              ? 'bg-[#F2EADA] text-[#4B2F1A] border-r-4 border-[#4B2F1A]'
              : 'text-gray-600 hover:bg-gray-50'
          } ${isRTL ? 'flex-row-reverse text-right' : 'text-left'}`}
        >
          <img src={cat.icon} alt="" className="w-6 h-6 object-contain" />
          <span className="flex-1">{t(`settings.${cat.labelKey}`)}</span>
        </button>
      ))}
    </div>
  );
